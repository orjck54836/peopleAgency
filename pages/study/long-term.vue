...
const sectionMeta = [
  ...
  { id: 'process', listCount: 4 },
  { id: 'cost', listCount: 3 },
  { id: 'documents', listCount: 3 },
  { id: 'howWeHelp', listCount: 0 },
]

const sections = computed(() =>
  sectionMeta.map(meta => ({
    id: meta.id,
    title: t(`longTerm.sections.${meta.id}.title`),
    body: t(`longTerm.sections.${meta.id}.body`),
    list: meta.listCount > 0
      ? Array.from({ length: meta.listCount }, (_, j) => ({
        title: t(`longTerm.sections.${meta.id}.list.${j}.title`),
        desc: t(`longTerm.sections.${meta.id}.list.${j}.desc`),
      }))
      : null,
  }))
)
...
const chips = computed(() =>
  Array.from({ length: 5 }, (_, i) => t(`longTerm.chips.${i}`))
)
...
<template>
  ...
  <section class="mode-hero">
    <div class="mode-hero-inner">
      <p class="mode-tag">{{ $t('longTerm.hero.tag') }}</p>
      <h1 v-html="$t('longTerm.hero.title')" />
      <p class="mode-sub">{{ $t('longTerm.hero.sub') }}</p>
    </div>
  </section>
  ...
  <main class="mode-wrapper">
    ...
    <section class="mode-schools">
      <h2 class="mode-schools-title">{{ $t('longTerm.schools.title') }}</h2>
      <p class="mode-schools-sub">{{ $t('longTerm.schools.sub') }}</p>
      <div v-if="recommendedSchools.length" class="mode-schools-grid">
        <div v-for="school in recommendedSchools" :key="school.name" class="school-card"
          ...
          <div class="school-card-body">
            <h3>{{ school.name }}</h3>
            <p>{{ school.location }}</p>
            <span class="school-card-link">{{ $t('longTerm.schools.detail') }}</span>
          </div>
        </div>
      </div>
      <div class="mode-schools-cta">
        <button class="btn-more" @click="goToAllSchools">
          {{ $t('longTerm.schools.viewAll') }}
        </button>
      </div>
    </section>
  </main>
