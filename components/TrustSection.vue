...
.trust-row--right .trust-cell--text {
  border-left: 1px solid var(--c-border);
}

.trust-row--left .trust-cell--reveal {
  border-left: 1px solid var(--c-border);
}
...
/* hover row：箭頭觸發動畫 */
.trust-row:hover .tri-arrow polygon {
  fill: var(--c-primary);
}

.trust-row:hover .tri-arrow .arrow-bottom {
  transform: translateY(-18px);
}

.trust-row:hover .tri-arrow .arrow-top {
  transform: translateY(18px);
}

/* hover 後箭頭淡出，內容淡入 */
.trust-row:hover .arrow-wrap {
  opacity: 0;
  transition: opacity 0.15s ease 0.1s;
}

.trust-row:hover .trust-cell--reveal {
...
.trust-row--right .reveal-content {
  transform: translateX(-10px);
}

.trust-row:hover .reveal-content {
  opacity: 1;
